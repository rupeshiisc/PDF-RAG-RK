...
def get_llm(selected_model: str):
    huggingface_model_ids = [
        "mistralai/Mistral-7B-Instruct-v0.1",  # Add others if needed
    ]
    
    if selected_model in huggingface_model_ids:
        logger.info(f"Using Hugging Face model: {selected_model}")
...
def extract_model_names():
    try:
        ...
        model_names = tuple(
            ...
            if model.get("name") and model["name"] != "llama2"
        )
        return model_names
...
def main():
    st.subheader("🧠 Ollama Chat with PDF RAG -- Nidhi Gupta", divider="gray", anchor=False)
